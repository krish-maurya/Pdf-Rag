...
function deterministicVector(text: string, dim = 768): number[] {
...
async function generateAnswer(query: string, contextDocs: any[]): Promise<string> {
  const googleKey = process.env.GOOGLE_API_KEY || process.env.GOOGLE_GEMINI_KEY;
  const openaiKey = process.env.OPENAI_API_KEY;
...
if (runWorker) {
  try {
    ...
    console.warn('Could not auto-start worker.js:', (err as Error).message);
  }
}
